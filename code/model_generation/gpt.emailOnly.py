...
config_path = os.path.join(base_dir, f"config.{language}.emailOnly.json")
with open(config_path, "r", encoding="utf-8") as f:
    cfg = json.load(f)

# Required config
names_by_group: Dict[str, List[str]] = cfg["names"]              # {"BF":[...], "WM":[...]}
inst_by_tier: Dict[str, List[str]] = cfg["domains"]         # {"TOP":[...], "LOW":[...]}
...
percorso_questionari = os.path.join(base_dir, f"questionnaire.{language}.emailOnly.txt")
percorso_questionari_noAttr = os.path.join(base_dir, f"questionnaire.{language}.emailOnly.noAttr.txt")
...
def render_questionnaire_attributed(
    ...
    q = q.replace("{domain}", str(inst_val))
...
def questionario_LLM(questionario_autore, prompt, client, TEMPERATURE=1.0) -> str:
    ...
    def clean(text):
        ...
        no_trailing_pipes = no_double_quotes.rstrip('|')
        return no_trailing_pipes
    ...
    def normalize_q_keys(s: str) -> str:
        ...
        return re.sub(r'\b([qQ])0*([0-9]{1,2})=', repl, s)
    ...
    while True:
        try:
            ...
            risposta = normalize_q_keys(clean(response.choices[0].message.content.strip()))
            break
...
for storia, testoStoria in storie_items:
    ...
    for group, tier in CONDITIONS:
        cond_id = f"{group}_{tier}"
    ...
    for cond_id, questAutore in autori_items:
        ...
        while n_ok < NUMBER_OF_ITERATIONS:
            ...
            risposta_str = questionario_LLM(questAutore, prompt_tpl, client, TEMPERATURE)
